...
function PreAttackEffect:update(dt)
	...
	if self.shooter and not self.shooter.dead then
		if self.back then
			self.x = self.shooter.x + 1.4*self.shooter.w*math.cos(self.shooter.collider:getAngle() - math.pi)
			self.y = self.shooter.y + 1.4*self.shooter.w*math.sin(self.shooter.collider:getAngle() - math.pi)
		else
