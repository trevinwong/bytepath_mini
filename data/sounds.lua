function sound()
    ...
    register('computer_startup', 'static', {'master', 'sfx', 'computer'})
    register('computer_background', 'static', {'master', 'sfx', 'computer'})
    ...
    register('game_hurt_small', 'static', {'master', 'sfx', 'game'})
...
function soundUpdate(dt)
    ...
    for i = #playing_sources, 1, -1 do
        -- source:isStopped() does not exist anymore in 11.0
        if not playing_sources[i].source:isPlaying() then
            if M.any(playing_sources[i].tags, 'music') then playRandomSong() end
    ...
    for _, source in ipairs(playing_sources) do
        if muted then 
            if M.any(source.tags, 'music') then
                source.source:stop() 
            end
        end
        source.source:setPitch(slow_amount)
        if M.any(source.tags, 'music') or M.any(source.tags, 'game') then
...
function playRandomSong()
    ...
    currently_playing_song = play(table.random(songs), {volume = 0.1})
end
...
function playGameHurtSmall()
    play('game_hurt_small', {volume = 1}) 
end

function playGameAmmo()
    play('game_ammo', {volume = 0.5, pitch = 1}) 
end

function playGameEngine()
    play('game_engine', {volume = 0.5, pitch = 1}) 
end
